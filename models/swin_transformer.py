# Adapted from Pytorch implementation of Swin Transformer
...
class PatchMergingV2(nn.Module):
    ...
    def __init__(self, dim: int, norm_layer: Callable[..., nn.Module] = nn.LayerNorm):
        ...
        self.norm_skip = norm_layer(2 * dim)  # difference
...
class SwinTransformerBlockV2(SwinTransformerBlock):
    ...
    def forward(self, x: Tensor, skip = False):  
        if self.skippable == False and skip == True:
            x = x + self.norm1_skip(self.attn(x))
            x = x + self.norm2_skip(self.mlp(x))
        else:
...
def swin_t(*, weights = None, progress: bool = True, **kwargs: Any) -> SwinTransformer:
    return _swin_transformer(
        ...
        embed_dim=96,
        depths=[2, 2, 6, 2],
        depths_base=[1, 1, 3, 1], # default: 4 skippable stages
        num_heads=[3, 6, 12, 24],
        window_size=[7, 7],
...
def swin_v2_t(*, weights = None, progress: bool = True, **kwargs: Any) -> SwinTransformer:
    return _swin_transformer(
        patch_size=[4, 4],
        embed_dim=96,
        depths=[2, 2, 6, 2],
        ...
        num_heads=[3, 6, 12, 24],
        window_size=[8, 8],
        # stochastic_depth_prob=0.2, # pytorch default
        stochastic_depth_prob=0.1,
        weights=weights,
        progress=progress,
