...
class SkippableConvNormActivation(torch.nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        if norm_layer is not None:
            ...
            if (skippable == False):
                self.norm_skip  = norm_layer(out_channels)

        if activation_layer is not None:
            params = {} if inplace is None else {"inplace": inplace}
            self.activation_layer = activation_layer(**params)
        else:
    ...
    def forward(self, x, skip=False) -> Tensor:
        x = self.conv_layer(x)
        if skip == True:
            x = self.norm_skip(x)
            # print("skip norm")
...
class SkippableConv2dNormActivation(SkippableConvNormActivation):
    """
