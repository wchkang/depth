...
def set_weight_decay(
    ...
    learning_rate: Optional[float] = None, # exp. woochul. 2024.06.08
):
    if not norm_classes:
        norm_classes = [
    ...
    for key in params:
        if len(params[key]) > 0:
            if learning_rate is not None: # exp. woochul. 2024.06.08
                lr = learning_rate
                param_groups.append({"params": params[key], "weight_decay": params_weight_decay[key], "lr":lr})
            else:
